...
A Next.js 15 (App Router) starter for B2B SaaS products: organizations,
team invites, and role-based access control, with tenant isolation
enforced in the data layer — not just hidden in the UI.

## Stack

- **Next.js 15** (App Router, Route Handlers, Edge middleware, async request APIs)
...
- **Tailwind CSS** + **lucide-react** for icons, no full component library
...
- Inviting someone in as an ADMIN requires being an OWNER yourself — an
  ADMIN can only invite people in as MEMBER (OWNER isn't an invitable
  role at all; it's reached only by promoting an existing member).
- Removing an OWNER from the workspace requires being an OWNER yourself,
  regardless of how many owners remain — an ADMIN can remove peers and
  MEMBERs, but never an OWNER.
