...
export default function RegisterForm() {
  ...
  return (
    <div className="card p-8">
      <h1 className="mb-1 font-display text-2xl font-medium">Create your account</h1>
      <p className="mb-7 text-sm text-ink/55">Start a new workspace in under a minute.</p>

      <form onSubmit={onSubmit} className="space-y-4">
        <div>
          <label className="field-label" htmlFor="name">Full name</label>
          <div className="relative">
            <User className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-ink/35" strokeWidth={1.75} />
            <input
              id="name"
              autoComplete="name"
              required
      ...
      <p className="mt-6 text-center text-sm text-ink/55">
        ...
        <Link href="/login" className="font-medium text-ink underline decoration-ink/25 underline-offset-4 hover:decoration-ink">
          Sign in
        </Link>
      </p>
    </div>
